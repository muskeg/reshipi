...
class TestDocsStructure:
    ...
    def test_translations_exist_for_all_base_docs(self, locale):
        """Every base doc should have a translation file for each non-default locale."""
        missing = []
...
class TestBuild:
    ...
    @pytest.mark.parametrize("locale", NON_DEFAULT_LOCALES)
    def test_locale_index_exists(self, site_dir, locale):
        assert (site_dir / locale / "index.html").is_file(), \
            f"Missing index.html for locale '{locale}'"
    ...
    @pytest.mark.parametrize("locale", NON_DEFAULT_LOCALES)
    def test_locale_recipe_pages_built(self, site_dir, locale):
        locale_dir = site_dir / locale
        assert locale_dir.is_dir(), f"Locale directory '{locale}' not found"
        html_files = list(locale_dir.rglob("*.html"))
        # Should have more than just index.html (at least recipe pages too)
        assert len(html_files) >= 2, \
            f"Locale '{locale}' has only {len(html_files)} HTML files, expected recipe pages too"
